...
def test_store_uses_exact_hdiutil_create_and_mount_commands(tmp_path: Path) -> None:
    ...
    hardened: list[Path] = []
    store = CaseSensitiveReleaseStore(cell, image, mount, runner=runner, case_sensitive_probe=lambda _: True, materialized_probe=lambda *_: True, permission_hardener=hardened.append)
    ...
    assert hardened == [image]
    assert runner.argv == [
        ("/usr/bin/hdiutil", "create", "-type", "SPARSEBUNDLE", "-fs", "Case-sensitive APFS", "-size", "12g", "-volname", "HermesPlatformRuntimeV1", str(image)),
...
def test_store_fails_closed_on_command_case_or_scope_mismatch(tmp_path: Path) -> None:
    ...
    store = CaseSensitiveReleaseStore(
        ...
        permission_hardener=lambda _: None,
    )
